...
class DataJunction(object):
    ...
    def get_generator(self):
        ...
        if self.runner is None:
            extra_args = {'ports' : list(map(itemgetter(1),
                                        self.input_sources))}
...
class Node(object):
    ...
    def prepare_splitter(self, splitter_function, size):
        ...
        if "size" in args:
            kwargs['size'] = size
        if "ports" in args:
            kwargs['ports'] = list(map(operator.attrgetter('name'),
                                  self.output_ports))
                ...
                try:
                    ...
                    if up_val in BYPASS_VALS:
                        ...
                        if self.bypass_buffer:
                            ...
                            yield next(flow) # will stop on stopiteration...
    ...
    def get_output_buffers(self):
        if not self.is_consumer:
            if len(self.output_ports) > 1 and 'out' in self.splitter.out_array_lookup:
                return list(map(len, self.splitter.out_array_lookup['out']))
...
class Network(object):
    ...
    def consumer_nodes(self):
        ...
        return list(filter(attrgetter('is_consumer'), list(self.node_objects.values())))
    
    @property
    def consumers(self):
        """ Return the outputs from the consumer Node objects """
        return [node.get_output() for node in self.consumer_nodes]
    
    @property
    def producer_nodes(self):
        """ Return the producers Node objects """
        return list(filter(attrgetter('is_producer'), list(self.node_objects.values())))
